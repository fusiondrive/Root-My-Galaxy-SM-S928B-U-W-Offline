...
firmware. One APK bundles two independently audited targets:
...
kernel `6.1.145` plus the DZF2 kernel release and build token. `SM-S928W`,
`SM-S928N`, `SM-S9280`, and any non-DZF2 build are rejected.

## Screenshots

These screenshots come from the two validated DZF2 devices. Root is per-boot.
...
<table>
  <tr>
    ...
    <td align="center"><strong>SM-S928B Root Checker</strong></td>
  </tr>
  <tr>
    <td align="center">
    ...
    <td align="center">
      <img src="docs/assets/screenshots/s928b-rootchecker.png" alt="Root Checker on SM-S928B DZF2" width="180">
    </td>
  </tr>
</table>
...
The two kernels share BTF and most symbol offsets. They are still separate
ports. The S928B profile changes the firmware identity and
`SLIDE_NFULNL_LOGGER_OFF` (`0x016a622a` vs `0x016a61b8` on U/U1). The KernelSU
modules have different vermagic strings and must not be swapped.
...
3. **Install [Shizuku](https://shizuku.rikka.app/).** Both ports were validated
...
```text
dist/RootMyGalaxy-S928-DZF2-offline-v0.3.0.apk
```
